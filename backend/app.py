...
def _emit_call_event(event_name, from_user_id, to_user_id, payload=None):
    data = {"from_user_id": int(from_user_id), "to_user_id": int(to_user_id)}
    if payload:
        data.update(payload)
    emit(event_name, data, room=f"user_{int(to_user_id)}")
...
def handle_call_invite(data):
    ...
    call_type = ((data or {}).get("call_type") or "video").strip().lower()
    if not me or not to_user_id or to_user_id == int(me):
        return
    if call_type not in {"audio", "video"}:
        call_type = "video"
    _emit_call_event("call_invite", me, to_user_id, {"call_type": call_type})
...
def handle_call_accept(data):
    me = session.get("user_id")
    to_user_id = int((data or {}).get("to_user_id") or 0)
    if not me or not to_user_id:
        return
    _emit_call_event("call_accept", me, to_user_id)
...
def handle_call_reject(data):
    me = session.get("user_id")
    to_user_id = int((data or {}).get("to_user_id") or 0)
    reason = ((data or {}).get("reason") or "rejected").strip()
    if not me or not to_user_id:
        return
    _emit_call_event("call_reject", me, to_user_id, {"reason": reason})
...
def handle_call_offer(data):
    ...
    to_user_id = int((data or {}).get("to_user_id") or 0)
    sdp = (data or {}).get("sdp")
    if not me or not to_user_id or not sdp:
        return
    _emit_call_event("call_offer", me, to_user_id, {"sdp": sdp})
...
def handle_call_answer(data):
    ...
    if not me or not to_user_id or not sdp:
        return
    _emit_call_event("call_answer", me, to_user_id, {"sdp": sdp})
...
def handle_call_ice(data):
    ...
    if not me or not to_user_id or not candidate:
        return
    _emit_call_event("call_ice", me, to_user_id, {"candidate": candidate})
...
def handle_call_end(data):
    me = session.get("user_id")
    to_user_id = int((data or {}).get("to_user_id") or 0)
    reason = ((data or {}).get("reason") or "ended").strip()
    if not me or not to_user_id:
        return
    _emit_call_event("call_end", me, to_user_id, {"reason": reason})
