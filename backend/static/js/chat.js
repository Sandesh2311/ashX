...
(() => {
  ...
  const endCallBtnTop = $('endCallBtnTop');
  ...
  const OFFLINE_KEY_PREFIX = `ashx_cache_${me.id}_`;
  const QUEUE_KEY = `ashx_queue_${me.id}`;
  ...
  async function setupPeerConnection(toUserId) {
    ...
    peerConnection.onicecandidate = (event) => {
      ...
      socket.emit('call_ice', { to_user_id: toUserId, candidate: event.candidate });
    };
  ...
  async function startOutgoingCall(type) {
    ...
    try {
      callType = type;
      currentCallPeerId = activePeer.id;
      callPeerName.textContent = activePeer.username;
      await setupPeerConnection(activePeer.id);
      callModal.classList.remove('hidden');
      socket.emit('call_invite', { to_user_id: activePeer.id, call_type: type });
    } catch (err) {
      alert('Unable to start call. Check mic/camera permissions.');
      resetCallState();
  ...
  async function acceptIncomingCall() {
    ...
    try {
      ...
      currentCallPeerId = incomingCall.from_user_id;
      const peer = contacts.find((c) => c.id === currentCallPeerId);
      callPeerName.textContent = peer ? peer.username : 'Call';
      await setupPeerConnection(currentCallPeerId);
      incomingCallPopup.classList.add('hidden');
      callModal.classList.remove('hidden');
      socket.emit('call_accept', { to_user_id: currentCallPeerId });
    } catch (_) {
      socket.emit('call_reject', { to_user_id: incomingCall.from_user_id, reason: 'media-error' });
      incomingCallPopup.classList.add('hidden');
      resetCallState();
    }
  }
  ...
  function rejectIncomingCall() {
    if (!incomingCall) return;
    socket.emit('call_reject', { to_user_id: incomingCall.from_user_id, reason: 'rejected' });
    incomingCallPopup.classList.add('hidden');
    incomingCall = null;
  }
  ...
  async function createAndSendOffer(toUserId) {
    ...
    socket.emit('call_offer', { to_user_id: toUserId, sdp: offer });
  }

  async function endCurrentCall(reason = 'ended') {
    if (currentCallPeerId) socket.emit('call_end', { to_user_id: currentCallPeerId, reason });
    callModal.classList.add('hidden');
    incomingCallPopup.classList.add('hidden');
    resetCallState();
  }
  ...
  socket.on('call_invite', ({ from_user_id, call_type }) => {
    const peer = contacts.find((c) => c.id === from_user_id);
    incomingCall = { from_user_id, call_type: call_type || 'video' };
    incomingCallerName.textContent = peer ? `${peer.username} is calling...` : 'Incoming call';
    incomingCallType.textContent = (call_type || 'video') === 'audio' ? 'Voice call' : 'Video call';
    incomingCallPopup.classList.remove('hidden');
  });

  socket.on('call_accept', async ({ from_user_id }) => {
    if (!peerConnection || !currentCallPeerId || from_user_id !== currentCallPeerId) return;
    await createAndSendOffer(from_user_id);
    startCallTimer();
  });

  socket.on('call_reject', ({ from_user_id, reason }) => {
    if (currentCallPeerId && from_user_id === currentCallPeerId) {
      alert(`Call ${reason || 'rejected'}`);
      callModal.classList.add('hidden');
      resetCallState();
    }
  });

  socket.on('call_offer', async ({ from_user_id, sdp }) => {
    try {
      if (!peerConnection || !currentCallPeerId) {
        callType = 'video';
        currentCallPeerId = from_user_id;
        await setupPeerConnection(from_user_id);
        callModal.classList.remove('hidden');
      }
      await peerConnection.setRemoteDescription(new RTCSessionDescription(sdp));
      const answer = await peerConnection.createAnswer();
      await peerConnection.setLocalDescription(answer);
      socket.emit('call_answer', { to_user_id: from_user_id, sdp: answer });
      startCallTimer();
    } catch (_) {
      endCurrentCall('error');
    }
  });

  socket.on('call_answer', async ({ from_user_id, sdp }) => {
    if (!peerConnection || from_user_id !== currentCallPeerId) return;
    await peerConnection.setRemoteDescription(new RTCSessionDescription(sdp));
  });

  socket.on('call_ice', async ({ from_user_id, candidate }) => {
    if (!peerConnection || from_user_id !== currentCallPeerId) return;
    try {
  ...
  socket.on('call_end', ({ from_user_id }) => {
    if (currentCallPeerId && from_user_id === currentCallPeerId) {
      callModal.classList.add('hidden');
      resetCallState();
    }
  ...
  endCallBtnTop.addEventListener('click', () => endCurrentCall('ended'));
